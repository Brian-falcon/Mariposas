...
export function SumWithDots({ activity }: { activity: Activity }) {
  ...
  if (result !== null) {
    ...
    return (
      <ActivityFeedback
        correct={result}
        correctMessage={`¡Muy bien! ${round.a} + ${round.b} = ${round.answer} 🎉`}
    ...
    <div className="p-6">
      {rounds.length > 1 && (
        <p className="text-lg text-center text-gray-500 mb-2">
          Suma {currentRound + 1} de {rounds.length}
        </p>
      )}
      <p className="text-xl text-center mb-6">
        Cuenta los puntitos y elige el resultado
      </p>
